...
contract Gateway is Ownable {
    ...
    function deployIdentityWithSalt(address identityOwner, string memory salt, uint256 signatureExpiry, bytes calldata signature) external returns (address) {
        ...
        address signer = ECDSA.recover(
            toEthSignedMessageHash(keccak256(abi.encode("Authorize ONCHAINID deployment", identityOwner, salt, signatureExpiry))),
            signature
        );
    ...
    function deployIdentityWithSaltAndManagementKeys(
        ...
        address signer = ECDSA.recover(
            toEthSignedMessageHash(keccak256(abi.encode("Authorize ONCHAINID deployment", identityOwner, salt, managementKeys, signatureExpiry))),
            signature
        );
