...
contract Verifier is Ownable {
    ...
    function removeTrustedIssuer(IClaimIssuer trustedIssuer) public onlyOwner {
        ...
        for (uint256 claimTopicIndex = 0; claimTopicIndex < trustedIssuerClaimTopics[address(trustedIssuer)].length; claimTopicIndex++) {
        ...
        for (claimTopic = 0; claimTopic < requiredClaimTopics.length; claimTopic++) {
            ...
            for (uint256 j = 0; j < claimIds.length; j++) {
                ...
                if (foundClaimTopic == requiredClaimTopics[claimTopic]) {
                    try IClaimIssuer(issuer).isClaimValid(IIdentity(identity), requiredClaimTopics[claimTopic], sig, data) returns (bool _validity) {
                        if (_validity) {
                            j = claimIds.length;
                        }
